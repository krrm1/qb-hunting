  ...
  QBCore.Functions.TriggerCallback('QBCore:HasItem', function(hasItem)
    if hasItem then
      if DoesEntityExist(baitAnimal) then
          ...
          local finished = exports["progressbars-draw"]:drawBar(5000,"Skinning")
          Wait(4300)
  ...
  for k,v in pairs (HuntingZones) do
    if IsEntityInZone(PlayerPedId(),v) then
      if cooldown <= 0 then
        ...
        LoadAnimDict('amb@medic@standing@kneel@base')
        TaskPlayAnim(PlayerPedId(), "amb@medic@standing@kneel@base" ,"base" ,8.0, -8.0, -1, 1, 0, false, false, false )
        local finished = exports["progressbars-draw"]:drawBar(5000,"Placing Bait",false,false,playerVeh)
        Citizen.Wait(100)
